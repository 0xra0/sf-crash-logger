...
- **Stack-overflow reporting** — a thread that has just overflowed keeps under 2 KiB of committed stack: far too little for dbghelp and a minidump, and on Wine/Proton the top-level exception filter is never even reached, so the crash produces **nothing at all**. (`SetThreadStackGuarantee`, the documented remedy, returns success under Wine and commits nothing.) The report is therefore handed to a dedicated reporter thread, created at load and holding a fresh 1 MiB stack, while the crashing thread blocks in a bounded wait — which also holds its stack still for the walk. Everything that identifies the crashing thread is captured before the handoff, so the `Thread:` line and the minidump's selected thread name the victim, not the reporter
- **Stack scan** — linearly scans raw stack memory for return addresses and object pointers, recovering the deeper frames the strict unwind-data walk drops in optimised engine code (no external tooling needed)
